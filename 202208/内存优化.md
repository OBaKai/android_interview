...
```java
一、并不会考虑到性能问题，不会全量开启KOOM。通过后台配置判断是否需要开启。

二、KOOM无法满足需求：
问题：
由于KOOM是通过阈值的方式监控内存的，然后分析Hprof然后到出一个Json格式的信息文件，用于上报内存问题的。
这样导致了如果发生了泄漏我们无法对泄漏源做一些补救措施。
比如一个Activity泄漏了，我们希望将泄漏的成本降到最低，将Activity内所有的View，Listener、Drawable 等可能存在的引用关系切断。只泄漏个空壳Activity（参考Matrix - ResourceCanary - ActivityLeakFixer）
  
解决：改造KOOM
1、在KOOM分析hprof阶段，它会帮我们分析是否有Activity、Fragment存在泄漏的，这个阶段我们将发现的Activity、Fragment 名称缓存起来，最后进行泄漏回调通知业务层。

2、我们业务层在 Application#registerActivityLifecycleCallbacks 在 Activity 回调 onDestory 的时候缓存起来（弱引用）
...
3、当收有泄漏回调通知，通过上报的 Activity name 从 map里边获取该Activity对象，然后进行 ResourceCanary - ActivityLeakFixer 处理
```
