...
增量更新：不管key是否重复，都直接将数据追加在前数据后
例如：a=1,b=3 增加a=5后变成了 a=1,b=3,a=5
因为读取的时候都是一个个读的 先a=1,再b=3,再a=5，前面的a=1会被a=5覆盖。

引发新问题：增量更新，不会导致数据越来越大吗？
当文件大小不够，会进行一次全量写入（去除重复key后，全量写入）。
如果文件大小满足写入大小，则直接全量写入。如果不满足，进行文件扩容（扩容根据平均k-v大小计算未来可能需要的文件大小进行扩容）

怎么mmkv扩容的？
	开启内存映射逻辑：
		fd = open("xxx.txt")
		ftruncate(fd，m_size) //设置文件大小
		mmap(0, m_size, xxx, fd, 0) //映射内存空间

	扩容：先关闭fd内存映射，fd扩容，再重新映射fd
		munmap(xxx)
		ftruncate(fd，m_size * 2)
		mmap(0, m_size, xxx, fd, 0)
