...
1.1 序列化/反序列化效率更高
① 参与序列化的属性信息的量更少
② 某些类型的属性使用小端序排列字节，充分利用了CPU读取内存中的数据的习惯（CPU读取数据都是小端序的）
其他暂时没想到了
...
```java
Varints数据压缩算法：一种将整数用1个或者多个字节表示的一种序列化方法，越小的数使用越少的字节。
例如：java中int占4个字节，假设一个int值为1，那么它也是占用4个字节就浪费空间了。Varints可以让它只占用一个字节。
...
也就是说使用varints算法的字节，一个字节只能表示0-128之间的数。更大的数就需要增加字节。
```
...
#### pb使用了 varints + least significant group first（小端序）

```java
pb在整数中使用了varints数据压缩算法，上边有介绍varints。
pb对多字节数据使用了least significant group first（小端序）。
  
例如：例如300这个整数，在pb中是序列化后是 0xAC02
十六制：0xAC02
二进制：10101100 00000010
varints算法：去掉高位，得到 0101100 与 0000010
  1 0101100
  0 0000010
...
tag注意：tag占1字节，如果field_number超过了15，那么tag会变成占用2个字节
  原因：tag使用了varints算法，所以占用了高位，剩下7位可用。
...
#### value（如果wire_type是0，value值也使用了varints算法）
...
但是：
使用varints对32位负数进行编码，恒定占用5个字节
使用varints对64位负数进行编码，恒定占用10个字节
  
所以需要引入 ZigZag编码
ZigZag编码：对负数做一次变换将其映射为一个正数
变成正数之后就能够用varints算法进行优化了。
