...
```java
put、get操作，最好情况是O(1)，最差情况是O(n)

put操作的流程：
① 取模运算，算出数组下标
② 检查数组该下标是否有元素，如果没有，直接new一个entey节点插入到数组中 -> O(1)
③ 数组下标已经有元素，并且元素个数小于8。
   则调用equals方法，比较是否存在相同名字的key，不存在则new一个entry插入都链表尾部 -> O(n)
④ 数组下标已经有元素，并且元素个数大于8。
   则调用equals方法，比较是否存在相同名字的key，不存在则new一个entry插入都链表尾部 -> O(logn)
   红黑树查询的时间复杂度是O(logn)
```
