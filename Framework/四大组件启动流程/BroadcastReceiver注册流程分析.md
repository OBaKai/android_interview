...
```java
广播分发机制中，如何走的是并行分发，那么AMS压根不管你（走了binder调用就行了），直观给你发，不会要你给回复的。
而且binder接口也肯定是oneway的。就算是非oneway，去到app进程这边也是post到主线程的Handler才执行处理广播接收的。
所以你在处理广播的时候做什么耗时操作都没人管你。
  
但是广播走串行分发的时候就不行了，AMS是会等广播一个处理完才发下一个的。
```
